...
public class RSAUtils {
     ...
     */
    public RSAPem ToPEM(boolean convertToPublic) {
        return new RSAPem(publicKey, convertToPublic ? null : privateKey);
    }
     ...
     */
    public RSAUtils SwapKey_Exponent_D__Unsafe() throws Exception {
        RSAPem pem = ToPEM(false);
        if (pem.Key_D == null) {
            RSAUtils rsa = new RSAUtils(pem);
            //公钥当成私钥使用
        ...
        return new RSAUtils(pem.SwapKey_Exponent_D__Unsafe());
    }
    ...
    /**
     * 简版多语言支持，根据当前语言返回中文或英文，简化调用{@link RSAPem#T(String, String)}
     **/
    static private String T(String zh, String en) {
        return RSAPem.T(zh, en);
    }
    ...
    public byte[] Encrypt(String padding, byte[] data) throws Exception {
        try (ByteArrayOutputStream stream = new ByteArrayOutputStream()) {
            ...
            if (CType.indexOf("OAEP") != -1) {
                ...
                if (blockLen < 1) {
                    String min = "NaN";
                    if (sub > 0) min = (int) Math.pow(2, Math.ceil(Math.log(sub * 8) / Math.log(2))) + "";
    ...
    static private OAEPParameterSpec[] createOaepParam(String ctype, String[] outType, int[] outLen) {
        ...
        if (CType.indexOf("MD5") != -1) {
            isMd5 = true;
            CType = CType.replace("MD5", "SHA-128");//伪装成SHA简化逻辑
        }
     ...
     */
    public RSAUtils(int keySize) throws Exception {
     ...
     */
    public RSAUtils(String pemOrXML) throws Exception {
        RSAPem pem;
        if (pemOrXML.trim().startsWith("<")) {
            pem = RSAPem.FromXML(pemOrXML);
        } else {
            pem = RSAPem.FromPEM(pemOrXML);
        }
     ...
     */
    public RSAUtils(RSAPem pem) throws Exception {
        publicKey = pem.getRSAPublicKey();
        if (pem.hasPrivate()) {
            privateKey = pem.getRSAPrivateKey();
        }
